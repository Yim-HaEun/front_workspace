...
let currentPage=0;//현재 페이지를 나타내는 변수,초기값은 0으로 설정하여 첫번째 페이지를 표시함
...
displayImg(page) : page는 현재 페이지 번호를 나타냄
    ...
    함수를 호출하는 공간에서 page값을 전달하면 함수는 해당 페이지에 해당하는 이미지를 표시
displayImg(0) : 첫 번째 페이지에 해당하는 이미지 표시
displayImg(1) : 두 번째 페이지에 해당하는 이미지 표시
*/
function displayImg(page){//이미지 표시 함수의 시작
    const startIndex = page*imagesPerpage; //현재 페이지 번호('page')와 페이지당 이미지 수(imagePerpage)를 곱해서 시작 인덱스를 계산
                                            //페이지 내에서 표시할 이미지의 첫 번째  이미지 인덱스를 나타냄

    const endIndex = startIndex + imagesPerpage; //시작 인덱스에 페이지당 이미지 수를 더해서 종료 인덱스를 계산
                                                // 페이지 내에서 표시할 마지막 이미지 다음 인덱스를 나타냄

    const pageImages = images.slice(startIndex,endIndex);//시작 인덱스에 페이지당 이미지 수를 더해서 종료 인덱스를 계산
...
function updateButtons(){
    ...
    첫번째 페이지 일 경우에는 이전 버튼을 비활성화 시켜서 첫번째 페이지 일 경우에는 이전 버튼을 클릭하지 못하게 막는다.
    */
    prevButton.disabled = currentPage===0;
    /*
    (currentPage+1)*imagesPerpage = 다음 페이지에 해당하는 이미지의 첫번째 인덱스
    images.length = images 배열의 길이로, 페이지를 넘어갈 수 있는 이미지의 전체 수를 나타냄
    (currentPage+1)*imagesPerpage >= images.length : 다음 페이지에 해당하는 이미지가 다음페이지에 이미지가 더이상 없을 때 다음 버튼을 비활성화 하겠음
    */
...
prevButton.addEventListener('click',() =>{
    if(currentPage > 0){ //현재 페이지 번호가 0보다 클 경우에만 실행
        ...
        updateButtons();//이전 버튼이 첫번째 페이지에서 클릭하게되면 더 이상 이전버튼으로 가지못하도록 이전버튼을 비활성화
    }
});
nextButton.addEventListener('click',() =>{
    ...
    */
    if((currentPage+1)*imagesPerpage <images.length){//현재 페이지에서 다음 페이지로 이동할 수 있는지 확인 
        currentPage++; //현재 페이지의 번호를 증가시켜 다음 페이지로 이동하게 함
